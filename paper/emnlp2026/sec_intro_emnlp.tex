...
In particular, the score trace, named activations, and alert marker in
Figure~\ref{fig:motivation} map onto the headline comparisons,
ontology-control blocks, and timing-support diagnostics, respectively.
...
matched launchers.
We do not claim dataset-agnostic superiority, universal policy-level causality,
...
\paragraph{Claim tiers and reading order.}
The strongest claim is the governed semantic-interface design.
A3D is the clean flagship for that claim; DAD remains an explicit stress lens.
This is why the paper uses a compact read rule: first read the visual protocol
anchors, then the headline tables, then the support/stress
evidence blocks that are explicitly marked as bounded in their role.

\paragraph{What this paper asks you to read first.}
Before any table, read the anchor map in this order:
\begin{itemize}[leftmargin=1.4em,topsep=2pt,itemsep=1pt]
  \item Figure~\ref{fig:motivation}: what the interface does on a real case.
  \item Figure~\ref{fig:vis_bridge}: what the interface carries in CRS and concept geometry.
  \item Figure~\ref{fig:ontology_evolution_main}: how the ontology is evolved and constrained.
  \item The protocol map (Table~\ref{tab:protocol_map_main}): how evidence is separated.
  \item Figure~\ref{fig:ontology_family_main}: ontology governance lens.
  \item Figure~\ref{fig:intervention_gallery}: representative intervention and timing-intuition cases.
  \item Figure~\ref{fig:data_efficiency}: breadth check across data regime variation.
  \item The safety-utility picture (Figure~\ref{fig:safety_utility}): the final position on the AP--mTTA frontier.
  \item The DAD stress map (Figure~\ref{fig:dad_stress_summary}): where mechanism evidence compresses to a stable frontier band.
  \item Figure~\ref{fig:cross_dataset_bridge}: CRASH transfer artifacts for stress-mode intuition.
\end{itemize}
This order keeps the first read anchored on mechanism and controlled comparison
rather than on any single mixed table line.
...
changes downstream behavior under matched launchers, and whether the paper's
...
\begin{itemize}[leftmargin=1.4em,topsep=2pt,itemsep=1pt]
  ...
  \item We show, with matched-launcher controls, that ontology choice changes the AP--mTTA operating point on both DAD and A3D, making the semantic interface itself an empirical variable rather than a cosmetic report.
