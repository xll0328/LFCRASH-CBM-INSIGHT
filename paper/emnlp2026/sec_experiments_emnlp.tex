...
variants) under matched training protocols, explicit seed/checkpoint rules, trigger-source
...
experiments, we keep the training protocol fixed and vary only the ontology.
To avoid count ambiguity, we distinguish two assets throughout:
the governed compact release ontology (80 concepts from an 81-concept
post-polish pool) and the legacy historical-full benchmark vocabulary
(837 concepts).

\paragraph{Comparability note for cross-paper benchmarks.}
Several published baselines use different backbones, input preprocessing, or
training schedules. We therefore use benchmark tables as operating-point
context, while treating matched in-paper protocol blocks (especially
Table~\ref{tab:concept_sets}) as the primary evidence for our central claim.

\paragraph{Protocol discipline.}
We separate \emph{headline} results from \emph{controlled support} and
\emph{stress} analyses. This keeps the central ontology claim measurable and
prevents mixing heterogeneous checkpoints into one leaderboard narrative. We
use three evidence levels:
\begin{itemize}
  \item \textbf{Headline}: canonical AP/mTTA comparisons.
  \item \textbf{Controlled support}: matched ontology and ablation blocks under one training protocol.
  \item \textbf{Stress}: timing-transfer and robustness diagnostics that define boundaries.
  \item \textbf{Language-side audits}: pseudo-label concentration, paraphrase
    robustness, and family-level governance checks.
\end{itemize}
The role of each major result block is summarized in
Table~\ref{tab:protocol_map_main}.
\begin{table}[t]
\caption{Protocol and evidence map for the main paper. It separates headline prediction
from controlled support blocks so the table is not mistaken for one pooled leaderboard.}
...
Table~\ref{tab:concept_sets} & shared ontology protocol & one run per ontology/data pair & 1 each & classifier & ontology science \\
...
\TableStyleOff
\end{table}
\paragraph{Execution depth.}
Beyond singleton headline rows, we report a completed 18-cell seeded ontology
block (DAD/A3D $\times$ 3 ontology variants $\times$ 3 seeds), A3D headline
stability checks, DAD stress/hardening blocks, and trigger-source transfer
audits. This does not imply dataset-agnostic universality; it establishes a
controlled evidence surface for the semantic-interface claim.
...
\fcolorbox{MacaronRule}{MacaronCardBlue}{%
...
\end{minipage}}
\hfill
\fcolorbox{MacaronRule}{MacaronCardMint}{%
\begin{minipage}[t]{0.31\textwidth}
\textbf{Controlled support evidence}\\
\textbf{Use:} test ontology/mechanism under matched protocols\\
...
\end{minipage}}
\hfill
\fcolorbox{MacaronRule}{MacaronCardPeach}{%
\begin{minipage}[t]{0.31\textwidth}
...
\begin{table}[t]
\caption{[Controlled support evidence] Semantic-validity diagnostics for the
released ontology (DAD-500 audit slice). These metrics support inspectability
and partial semantic stability, but do not establish full causal faithfulness.}
\label{tab:semantic_validity}
\centering\footnotesize
\TableStyleOn
\setlength{\tabcolsep}{3pt}
\begin{tabular}{p{0.27\linewidth}p{0.21\linewidth}p{0.44\linewidth}}
\toprule
\TableHead
Diagnostic block & Key statistic & Reading \\
\midrule
Pseudo-label concentration (top-$m$) &
top-1: 1.00 family/frame, 5.48\% mass; top-3: 2.34 families/frame, 16.02\% mass;
top-10: 5.14 families/frame, 51.46\% mass &
top-3 remains a practical middle point between over-concentrated and overly
diffuse supervision. \\
Canonical paraphrase stability &
text cosine: 0.939; frame-score correlation: 0.887; top-10 overlap: 0.525;
mean absolute score diff: 0.0134 &
canonical names are reasonably stable under light wording variation, with
weaker behavior on abstract paraphrases. \\
Governance coverage and review traceability &
611 $\rightarrow$ 81 post-polish candidates; 80/80 released concepts reviewed;
9 families covered &
the ontology is traceable and governed as a released asset rather than an
unlogged phrase pool. \\
\bottomrule
\end{tabular}
\TableStyleOff
\end{table}
...
We also measured whether canonical release-ontology concept names remain stable
...
whether the release ontology is brittle to minor wording changes. Together
...
than as a hidden prompt artifact. At the same time, we do \emph{not} treat
these diagnostics as proof of full concept faithfulness; they are support
evidence for inspectability and semantic stability under bounded tests.
...
DE$^2$B~\citep{guan2025domain} & text-conditioned dual branch & timing score stream & partial & --\\
RARE~\citep{song2025rare}    & object-attention reuse & real-time score stream & partial & --\\
...
On A3D, \method{} achieves a stronger and cleaner flagship result. The
...
colored ontology markers show three-seed matched-protocol aggregates with
...
shared training protocol. Only
the ontology source varies; the training-protocol family is held fixed.}
...
\paragraph{Source vs.\ count caveat.}
Table~\ref{tab:concept_sets} changes both ontology source and concept count, so
its interpretation is a \emph{joint source+cardinality effect}, not a pure
source-only estimate. To isolate this, we launched a size-matched control track
with fixed 30/80 concept budgets (risk-core vs.\ historical-30, and perfect-v1
vs.\ historical-80) using deterministic, family-stratified subsets from the
historical vocabulary. Those controls are treated as support evidence and are
reported separately from headline tables once the seeded block completes.
...
recommended seeded runs are finished. On A3D, the released discovered
...
\paragraph{Effect-size view (seed-aggregated).}
To make the ontology claim explicit, we also report pairwise aggregate
differences with normal-approximate 95\% intervals. On DAD, risk-core manual
vs.\ historical full is $+1.36$ AP points (CI $[-0.72, +3.43]$), while perfect
v1 vs.\ historical full is $-4.05$ AP points (CI $[-12.48, +4.38]$) with
$+0.42$s mTTA (CI $[-0.35, +1.18]$). On A3D, perfect v1 vs.\ historical full
is $+1.67$ AP points (CI $[-0.03, +3.36]$), and risk-core manual vs.\ perfect
v1 shows a timing shift of $-1.65$s mTTA (CI $[-3.04, -0.26]$). The practical
reading is not that one ontology uniformly dominates; the effect is
dataset-dependent, and DAD remains the high-variance regime.
...
policy-level intervention timing account lies beyond the current paper.

\subsection{Ontology Quality as Evidence, Not Decoration}

The ontology work is not merely qualitative preprocessing. The release
...
Concretely, the release audit trail compresses 611 mined phrases into a
...
\caption{[Language-side support evidence] Family coverage in the released ontology. The polished concept
...
used consistently in both experiments and release audits.}
...
is not yet a finished human-verified concept benchmark or a complete
...
still weakens the cleaner release-ontology support family.
...
\begin{figure*}[t]
  ...
  \caption{\textbf{Canonical merge examples from the released ontology.}
