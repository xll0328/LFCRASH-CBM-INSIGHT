...
def box(x, y, w, h, title, body, color, title_fs=14, body_fs=11.2):
...
ax.text(0.35, 7.55, 'Risk Concept Discovery and Adjudication Pipeline', fontsize=20, fontweight='bold', color=palette['slate'])
ax.text(0.35, 7.08, 'From raw driving frames to trainable accident-risk ontology for CBM + Actor-Critic training', fontsize=12.5, color='#475569')
...
ax.text(7.9, 2.85, 'Semantic interface is constructed, not assumed', ha='center', fontsize=15, fontweight='bold', color=palette['slate'])
ax.text(7.9, 2.45, 'Discovered concepts are evaluated by trainability, auditability, and intervention readiness, not just readability.', ha='center', fontsize=11.5, color='#475569')

box(1.0, 0.55, 4.4, 1.35,
    'Example raw outputs',
    '"vehicle drifting into ego lane"\n"closing gap to front vehicle"\n"brake light onset"',
    '#6366f1', title_fs=12.5, body_fs=9.2)
box(5.8, 0.55, 4.4, 1.35,
    'Example refined primitives',
    'lane intrusion\ncut-in risk\nbraking onset / visibility degradation',
    '#7c3aed', title_fs=12.5, body_fs=9.2)
box(10.6, 0.55, 4.4, 1.35,
    'Output ontology families',
    'trajectory conflict\ndistance/proximity\nvisibility / control response',
    '#059669', title_fs=12.5, body_fs=9.2)
