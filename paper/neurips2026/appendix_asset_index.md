# Appendix Asset Index

This index links the paper's appendix claims to concrete repository assets.

## Ontology construction
- `paper/neurips2026/sec_appendix.tex`
- `paper/neurips2026/make_concept_pipeline_fig.py`
- `paper/neurips2026/make_concept_family_coverage_fig.py`
- `paper/neurips2026/make_concept_case_study_fig.py`
- `paper/neurips2026/make_concept_evolution_fig.py`
- `output/concept_sets/neurips2026_controlled_ontology_manifest.json`

## Intervention evidence
- `insight_concept_intervention.py`
- `paper/neurips2026/intervention_v4_backbone.py`
- `paper/neurips2026/intervention_protocol_and_casebank.md`
- `output/insight_analysis/dad_intervention_v4_hybrid_boost1/concept_intervention_summary.json`
- `paper/neurips2026/v4_case_bank_shortlist.json`

## Timing-faithfulness evidence
- `insight_timing_faithfulness.py`
- `paper/neurips2026/timing_faithfulness_package.md`
- `output/insight_analysis/dad_timing_v4_enhanced/timing_faithfulness_summary.json`

## Hard-case symmetry scaffold
- `paper/neurips2026/build_dad_hard_case_symmetry_audit.py`
- `paper/neurips2026/build_hard_case_pair_review_sheet.py`
- `paper/neurips2026/dad_hard_case_symmetry_audit.md`
- `paper/neurips2026/dad_hard_case_pair_review_draft_20260506.md`
- `paper/neurips2026/dad_hard_case_pair_review_sheet.md`
- `paper/neurips2026/validate_hard_case_symmetry_gate.py`
- `paper/neurips2026/hard_case_symmetry_gate_summary.json`
- `paper/neurips2026/v4_case_bank_scan.json`

## Result truthfulness and protocol separation
- `paper/neurips2026/claim_evidence_audit.json`
- `paper/neurips2026/submission_results_ledger.json`
- `paper/neurips2026/run_neurips_sanity_checks.py`
- `paper/neurips2026/neurips_sanity_report.md`
- `paper/neurips2026/reviewer_proof_experiment_manifest.md`
- `paper/neurips2026/threats_to_validity.md`
- `paper/neurips2026/compute_and_asset_notes.md`

## Writing / positioning assets
- `paper/neurips2026/PROJECT_STATUS_20260501.md`
- `paper/neurips2026/ORAL_BEST_PAPER_SPRINT_LEDGER_20260506.md`
- `paper/neurips2026/thesis_claims_evidence_matrix.md`
- `paper/neurips2026/rebuttal_map.md`
