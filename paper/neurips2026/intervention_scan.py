...
params = BEST_PARAMS['dad']; ds = DATASET_PARAMS['dad']
...
with torch.no_grad():
    for idx, (x, y, toa) in enumerate(loader):
        ...
        top = np.argsort((acts[max(0,int(toa.item())-40):int(toa.item())+1].mean(0)) * risk_w)[::-1][:8]
        top_names = []
        for k in top:
            s = names[k].strip()
            ...
            top_names.append(s)
        out.append({'idx':idx,'alert':alert,'tta':tta,'max_ac':float(ac.max()),'max_prob':float(probs.max()),'top_idx':top.tolist(),'top_names':top_names})

late = [o for o in out if o['tta'] < 0.5]
late = sorted(late, key=lambda z: z['max_ac'], reverse=True)
print(json.dumps({'num_pos':len(out),'late_top':late[:20]}, indent=2, ensure_ascii=False))
