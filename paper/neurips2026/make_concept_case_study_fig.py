...
palette = {'raw':'#b45309','merge':'#475569','final':'#0f766e','text':'#111827','muted':'#475569'}
ax.text(0.35, 7.95, 'Case studies of semantic compression and canonicalization', fontsize=19, fontweight='bold', color=palette['text'])
ax.text(0.35, 7.5, 'These examples show that the final ontology is not a prettier synonym list: it preserves risk semantics while removing wording noise and fragment duplication.', fontsize=11.5, color=palette['muted'])
...
for (title, raw_list, final_name), y in zip(cases, row_y):
    ...
    ax.text(0.7, y + 0.66, title, fontsize=11.8, fontweight='bold', color=palette['raw'])
    ax.text(0.7, y + 0.28, '  |  '.join(raw_list), fontsize=10.2, color=palette['text'])
    ...
    ax.text(7.7, y + 0.45, final_name, fontsize=11.1, color=palette['text'], va='center', fontweight='bold')
    ax.add_patch(FancyArrowPatch((10.95, y + 0.45), (12.2, y + 0.45), arrowstyle='-|>', mutation_scale=16, linewidth=2, color='#64748b'))
    ax.text(12.4, y + 0.45, 'Retained because it is shorter, canonical, and more intervention-ready.', fontsize=10.1, color=palette['muted'], va='center')
