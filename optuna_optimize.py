...
# 数据根目录（与train_best_params.py对齐）
DATA_ROOT = "/data/sony/LFCRASH/CRASH/data"


def suggest_hyperparameters(trial, dataset_name):
    """为给定数据集建议超参数"""
    if dataset_name == "dad":
        return {
            "lambda_align": trial.suggest_float("lambda_align", 1e-6, 1e-3, log=True),
        ...
        return {
            "lambda_align": trial.suggest_float("lambda_align", 1e-5, 1e-2, log=True),
...
def train_and_evaluate(trial, dataset_name, device, n_epochs=50, use_test_set=False):
    ...
    # 获取超参数
    params = suggest_hyperparameters(trial, dataset_name)
    ds_params = DATASET_PARAMS[dataset_name]
    
    # 加载数据集
    # 数据集路径（与train_best_params.py对齐，注意实际路径是小写）
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if epoch % 5 == 0 or epoch == n_epochs:
            ...
            with torch.no_grad():
                for x, y, toa in eval_loader:
                    ...
                    if len(all_outputs) > 0:
                        ...
                        all_pred.extend(video_probs.cpu().numpy())
            ...
            if len(all_pred) > 0:
                ...
                # evaluation函数期望frame-level predictions (n_videos, n_frames)
                # 我们需要将video-level predictions扩展为frame-level
                n_videos = len(all_pred)
                n_frames = ds_params["n_frames"]
                all_pred_frame = np.tile(all_pred[:, None], (1, n_frames))  # (n_videos, n_frames)
...
def main():
    ...
    parser.add_argument('--n_trials', type=int, default=200, help='试验次数（默认200，可运行24小时）')
    parser.add_argument('--n_epochs', type=int, default=50, help='每个trial的epoch数')
    parser.add_argument('--use_test_set', action='store_true',
                        help='使用测试集进行优化（用户允许）')
    parser.add_argument('--output_dir', type=str, default='output/optuna',
                        help='输出目录')
    parser.add_argument('--study_name', type=str, default=None,
                        help='Study名称（用于恢复）')
    parser.add_argument('--storage', type=str, default=None,
                        help='Optuna存储URL（用于分布式优化）')
    ...
    if args.study_name is None:
        args.study_name = f"optuna_{args.dataset}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    results_file = os.path.join(args.output_dir, f"{args.dataset}_optuna_results.json")
    ...
    # 保存study对象
    study_file = os.path.join(args.output_dir, f"{args.dataset}_optuna_study.pkl")
